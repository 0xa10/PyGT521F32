...
class SCSIInterface(object):
    ...
    def open(self):
        ...
        if drive_type not in (DRIVE_REMOVABLE, DRIVE_CDROM):
            raise SCSIException("Drive type is incorrect.")
        ...
        if self._port_handle == win32.INVALID_HANDLE_VALUE:
            ...
            raise SCSIException("Could not open drive.")
    ...
    def _scsi_operation(self, pbuf, size, read, timeout=10):
        ...
        if not result_ok and win32.GetLastError() not in (win32.ERROR_SUCCESS, win32.ERROR_IO_PENDING):
            raise SCSIException("DeviceIoControl failed ({!r})".format(ctypes.WinError()))

        if returned.value < (SCSI_PASS_THROUGH_DIRECT.ScsiStatus.offset + SCSI_PASS_THROUGH_DIRECT.ScsiStatus.size):
            raise SCSIException("Not enough SCSI information returned to determine error")

        if sptdwb.sptd.ScsiStatus != 0:
            raise SCSIException("SCSI Operation returned %d" % (sptdwb.sptd.ScsiStatus,))
